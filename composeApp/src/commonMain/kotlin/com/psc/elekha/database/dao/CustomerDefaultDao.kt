...
interface CustomerDefaultDao {
    ...
    fun insertCustomer(customerDefaultEntity: CustomerDefaultEntity)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    fun insertAllCustomer(customerDefaultEntity: List<CustomerDefaultEntity>?)

    @Query("Select * from CustomerDefault")
    fun getAllCustomer(): List<CustomerDefaultEntity>?

    @Query("Select * from CustomerDefault where ContactNo=:ContactNo and CutomerStatusID = 5")
    fun getCustomerByContactNumber(ContactNo: String): List<CustomerDefaultEntity>?
