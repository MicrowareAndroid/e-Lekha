...
interface MSTVillageDao {
    ...
    fun insertVillage(mSTVillageEntity: MSTVillageEntity)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    fun insertAllVillage(mSTVillageEntity: List<MSTVillageEntity>?)

    @Query("Select * from MSTVillage")
    fun getAllVillage(): List<MSTVillageEntity>?

    @Query("Select * from MSTVillage where BranchID=:BranchID and IsDeleted = 0")
    fun getAllVillageByBranchID(BranchID: Int): List<MSTVillageEntity>?
